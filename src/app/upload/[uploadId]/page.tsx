...
export default function Page() {
    ...
    const { data: segments = [] } = api.videos.segments.useQuery(uploadId as string, {
        enabled: !!uploadId,
        refetchInterval: 5_000,
    });
    ...
    const { data: { id: jobId } = {}, mutate: enqueue } = api.jobs.enqueue.useMutation();
    ...
    return (
        <div className="m-0 flex h-screen w-screen items-center justify-center bg-gray-900" style={{
            ...
            <div className="container mx-auto max-w-xl bg-gray-600 p-8">
                ...
                <div className="mb-4">
                    {isDone && video?.thumbnailUrl && <Image src={video?.thumbnailUrl} width={640} height={360} alt={video?.description!} />}
                </div>
                <div className="mb-4">
                    {isDone ? <Link href={`/watch/${video?.id}`} className="text-white underline hover:text-gray-300">Watch here</Link> : <Spinner />}
                </div>
                <ul className="grid grid-cols-3 gap-4">
                    {statusIcons.map(({ label, value, onClick }) => (
                        <li key={label} className={
                            `${onClick ? 'cursor-pointer' : ''} flex items-center justify-center rounded-lg bg-gray-500 p-4 shadow`
                        } onClick={onClick} >
